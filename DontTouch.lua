...
-- Create a local function to handle the animation script's lifecycle.
-- This makes it easy to re-run the entire script when the character respawns.
local function setupAnimationScript()
    -- Correctly find the player's character and wait for it to fully load
    local player = game.Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local v_u_1 = character

    -- Now that we have the character, we can safely find its parts
    ...
    function playAnimation(p53, p54, p55)
        ...
        if v58 ~= v_u_13 then
            ...
            -- New conditional to set jump animation priority to Action
            if p53 == "jump" then
                v_u_14.Priority = Enum.AnimationPriority.Action
            else
                v_u_14.Priority = Enum.AnimationPriority.Movement
            end
    ...
    function onRunning(p72)
        ...
        -- A more reliable check for when to play the walking animation
        if v_u_7.MoveDirection.Magnitude > 0.01 and v_u_8 ~= "Running" then
            playAnimation("walk", 0.1, v_u_7)
            if v_u_13 and v_u_13.AnimationId == "http://www.roblox.com/asset/?id=180426354" then
                -- MODIFIED LINE: Walk speed is now 0.8, regardless of humanoid speed
                setAnimationSpeed(0.8)
            end
            v_u_8 = "Running"
        -- Check if the character has stopped moving to play the idle animation
        elseif v_u_7.MoveDirection.Magnitude <= 0.01 and v_u_8 == "Running" then
            playAnimation("idle", 0.1, v_u_7)
            v_u_8 = "Standing"
        end
        
        -- ADDED CODE: Fixes the frozen animation issue when landing.
        if v_u_7.FloorMaterial ~= Enum.Material.Air and v_u_8 == "FreeFall" then
            stopAllAnimations()
        end
    end
    function onDied()
    ...
    function onJumping()
        ...
        setAnimationSpeed(1) -- This line has been changed to set the jump animation speed to 1.
        v_u_45 = 0.3
        v_u_8 = "Jumping"
    end
    ...
    function move(p81)
        ...
        if v_u_8 == "FreeFall" and v_u_45 <= 0 then
            ...
            -- The conflicting animation call has been removed from this block
            if v_u_8 == "Dead" or (v_u_8 == "GettingUp" or (v_u_8 == "FallingDown" or (v_u_8 == "Seated" or v_u_8 == "PlatformStanding"))) then
                stopAllAnimations()
                v85 = true
                v83 = 1
                v82 = 0.1
...
-- Re-run the script whenever the player's character is added (i.e., when they respawn).
game.Players.LocalPlayer.CharacterAdded:connect(setupAnimationScript)

-- Call the function initially to set up the script for the first time.
